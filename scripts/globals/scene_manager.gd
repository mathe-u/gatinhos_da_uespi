...
var multiplayer_menu_scene_path: String = "res://scenes/test/test_scene_multiplayer_menu.tscn"
...
func switch_to_main_game_scene() -> void:
	#var current_multiplayer_menu = get_tree().root.get_node(multiplayer_menu_root_path)
	#if current_multiplayer_menu:
		#current_multiplayer_menu.queue_free()

	# Also remove the existing main scene container itself, in case we are reloading
	# or switching back to it from a different state within the game loop.
	var current_main_scene = get_tree().root.get_node(main_scene_root_path)
	if current_main_scene:
...
func switch_to_multiplayer_menu_scene() -> void:
	...
	# Also remove the existing multiplayer menu itself, in case we are reloading it.
	#var current_multiplayer_menu = get_tree().root.get_node(multiplayer_menu_root_path)
	#if current_multiplayer_menu:
		#current_multiplayer_menu.queue_free()
	...
	# Load and add the new multiplayer menu scene
	get_tree().change_scene_to_file(multiplayer_menu_scene_path)
	print("loaded")
	#var new_multiplayer_menu: Node = load(multiplayer_menu_scene_path).instantiate()
	
	#if new_multiplayer_menu != null:
		#get_tree().root.add_child(new_multiplayer_menu)
		# Optional: Set the new scene as the current_scene if needed for input handling, etc.
		# get_tree().current_scene = new_multiplayer_menu
		#await get_tree().process_frame # Wait for it to be added
	#else:
		#print("Error: Could not instantiate multiplayer menu scene from path ", multiplayer_menu_scene_path)
